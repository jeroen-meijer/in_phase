...
class CrawlProgressDisplay {
  ...
  static _Layout _computeLayout(io.Stdout stdout, int sourceCount) {
    ...
    final recentCount = available > 4
        ? _recentCompletedCount.clamp(0, available - 4)
        : 0;
    final inProgressSlots = (available - 1 - recentCount - 1).clamp(
      1,
      sourceCount,
    );
  ...
  CrawlProgressReporter reporterFor(int index) =>
      (name) => setDisplayName(index, name);

  /// Updates the display name for source [index]. Call when the name is
  /// resolved.
  ...
  /// Adds to recently completed and reuses its slot for the next pending
  /// source.
  ...
  void _redraw() {
    ...
    if (_isFirstRedraw) {
      ...
      _stdout
        ..write('\r')
        ..write(_cursorUp(_layout.totalLines - 1));
    }
    ...
    _stdout
      ..write(_eraseLine)
      ..writeln(_formatHeader(completedCount));

    for (var r = 0; r < _layout.recentCount; r++) {
      _stdout
        ..write(_eraseLine)
        ..writeln(_formatRecentlyCompletedLine(r));
    }

    _stdout
      ..write(_eraseLine)
      ..writeln('─' * (_tagWidth + 15));
  ...
  String _formatCompletedSource(int sourceIndex) {
    final label =
        (_displayNames[sourceIndex] ?? _shortId(_identifiers[sourceIndex]))
            .padRight(_tagWidth);
...
typedef CrawlProgressReporter = void Function(String name);
